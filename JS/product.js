...
.then(response => response.json())
        .then(product => {
                ...
                + "<div class='article__price'>" + product.price + "€</div>"
                ...
                +   "<button type='button' onclick=''>Ajouter au panier</button>"
                + "</form>"
...
function addPanier(product, quantite) {
  ...
  for (let element of panierBefore) {
      ...
      if (element._id == product._id) {
          ...
          //Mettre a jour le local storage avec nouveau panier
          return
      }
  }
  //créer l'élément dans le local storage
  let element={
    "_id": product._id, 
    "prix": product.price,
    "quantité": quantite,
  ...
  localStorage.insert(JSON.stringify(element))
}
